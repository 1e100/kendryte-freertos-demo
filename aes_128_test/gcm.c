...
int mbedtls_gcm_setkey(mbedtls_gcm_context* ctx, mbedtls_cipher_id_t cipher,
  ...
  if (ret != 0) return ret;
  ret = mbedtls_cipher_setkey(&ctx->cipher_ctx, key, keybits, MBEDTLS_ENCRYPT);
  if (ret != 0) return ret;
...
int mbedtls_gcm_update(mbedtls_gcm_context* ctx, size_t length,
  ...
  while (length > 0) {
    ...
    for (i = 0; i < use_len; i++) {
      if (ctx->mode == MBEDTLS_GCM_DECRYPT) ctx->buf[i] ^= p[i];
      out_p[i] = ectr[i] ^ p[i];
      if (ctx->mode == MBEDTLS_GCM_ENCRYPT) ctx->buf[i] ^= out_p[i];
    }
...
int mbedtls_gcm_finish(mbedtls_gcm_context* ctx, unsigned char* tag,
  ...
  if (tag_len > 16 || tag_len < 4) return (MBEDTLS_ERR_GCM_BAD_INPUT);
...
int mbedtls_gcm_auth_decrypt(mbedtls_gcm_context* ctx, size_t length,
  ...
  ret = mbedtls_gcm_crypt_and_tag(ctx, MBEDTLS_GCM_DECRYPT, length, iv, iv_len,
...
int mbedtls_gcm_self_test(int verbose) {
  ...
  for (j = 0; j < 3; j++) {
    ...
    for (i = 0; i < MAX_TESTS; i++) {
      ...
      if (ret != 0 || memcmp(buf, ct[j * 6 + i], pt_len[i]) != 0 ||
