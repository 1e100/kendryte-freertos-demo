...
void aes_cpu_get_data(uint8_t* input_key, size_t key_len, uint8_t* iv,
  ...
  if (cipher_mod == AES_CBC) {
    ...
    aes_cbc256_hard_encrypt(&cbc_context, aes_data, data_size,
                            aes_hard_out_data);
    if (get_time_flag)
    ...
    aes_gcm256_hard_encrypt(&gcm_context, aes_data, data_size,
...
check_result_t aes_check_decrypt(uint8_t* input_key, size_t key_len,
  ...
  if (cipher_mod == AES_CBC)
    aes_cbc256_hard_decrypt(&cbc_context, aes_hard_out_data, padding_len,
                            aes_soft_out_data);
  else if (cipher_mod == AES_ECB)
    aes_ecb256_hard_decrypt(input_key, aes_hard_out_data, padding_len,
                            aes_soft_out_data);
  else
    aes_gcm256_hard_decrypt(&gcm_context, aes_hard_out_data, padding_len,
