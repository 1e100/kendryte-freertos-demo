  ...
  switch (event) {
    ...
    case VIDEO_FE_END:
      dvp_set_output_attributes(
          file_dvp, DATA_FOR_DISPLAY, VIDEO_FMT_RGB565,
          ctx->gram_mux ? ctx->lcd_image0->addr : ctx->lcd_image1->addr);
...
void dvp_init(camera_context_t* ctx) {
  ...
  dvp_set_output_attributes(file_dvp, DATA_FOR_DISPLAY, VIDEO_FMT_RGB565,
                            (void*)ctx->lcd_image0->addr);

  dvp_set_output_attributes(file_dvp, DATA_FOR_AI, VIDEO_FMT_RGB24_PLANAR,
                            (void*)ctx->ai_image->addr);
