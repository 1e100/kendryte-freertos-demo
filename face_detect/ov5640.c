...
const static uint8_t OV5640_EFFECTS_TBL[7][3] = {
    ...
    {0X1E, 0xA0, 0X40}, {0X1E, 0x80, 0XC0}, {0X1E, 0x80, 0X80},
    {0X1E, 0x40, 0XA0}, {0X40, 0x40, 0X10}, {0X1E, 0x60, 0X60},
};
...
// set the output size
uint8_t OV5640_OutSize_Set(uint16_t offx, uint16_t offy, uint16_t width,
                           uint16_t height) {
...
void jpeg_test(uint8_t jpg_size) {
  ...
  OV5640_JPEG_Mode();
  OV5640_OutSize_Set(4, 0, jpeg_size_tbl[jpg_size][0],
                     jpeg_size_tbl[jpg_size][1]);
  ...
  //        (uint32_t)jpeg_data_buf, jpeg_buf_size/4 );
...
void rgb565_test(void) {
  ...
  //        OV5640_WR_Reg(0x3035,0X51); // slow down OV5640 clocks ,adapt to the
  //        refresh rate of the LCD OV5640_WR_Reg(0x3036,0X88);
  //
  //        /* DCMI DMA DeInit */
  //        HAL_DMA_DeInit(&hdma_dcmi);
  //
