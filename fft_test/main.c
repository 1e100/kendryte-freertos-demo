 ...
 * FFT has nine layers, shift determines which layer needs shift operation to
 * prevent overflow. (for example, 0x1ff means 9 layers do shift operation; 0x03
 * means the first layer and the second layer do shift operation) If shifted,
 * the result is different from the normal FFT.
 */
...
uint16_t get_bit1_num(uint32_t data) {
  uint16_t num;
  for (num = 0; data; num++) data &= data - 1;
  return num;
}
...
int main(void) {
  ...
  for (i = 0; i < FFT_N / 2; i++)
    printf("%3d:%7d %7d %7d %7d\n", i, data_hard[i].real,
           (int32_t)data_soft[i].real, data_hard[i].imag,
           (int32_t)data_soft[i].imag);
  ...
  for (i = 0; i < FFT_N; i++) {
    hard_power[i] = sqrt(data_hard[i].real * data_hard[i].real +
                    ...
                    2;
    soft_power[i] = sqrt(data_soft[i].real * data_soft[i].real +
                         data_soft[i].imag * data_soft[i].imag) *
                    2;
  }
  printf("\nhard power  soft power:\n");
  printf("%3d : %f  %f\n", 0, hard_power[0] / 2 / FFT_N * (1 << bit1_num),
         soft_power[0] / 2 / FFT_N);
  for (i = 1; i < FFT_N / 2; i++)
    printf("%3d : %f  %f\n", i, hard_power[i] / FFT_N * (1 << bit1_num),
           soft_power[i] / FFT_N);
  ...
  printf("[hard ][%d bytes][forward time = %ld us][backward time = %ld us]\n",
         FFT_N,
         ((get_time[FFT_HARD][FFT_DIR_FORWARD][TEST_STOP].tv_sec -
           get_time[FFT_HARD][FFT_DIR_FORWARD][TEST_START].tv_sec) *
              1000 * 1000 +
          (get_time[FFT_HARD][FFT_DIR_FORWARD][TEST_STOP].tv_usec -
           get_time[FFT_HARD][FFT_DIR_FORWARD][TEST_START].tv_usec)),
         ((get_time[FFT_HARD][FFT_DIR_BACKWARD][TEST_STOP].tv_sec -
           get_time[FFT_HARD][FFT_DIR_BACKWARD][TEST_START].tv_sec) *
              1000 * 1000 +
          (get_time[FFT_HARD][FFT_DIR_BACKWARD][TEST_STOP].tv_usec -
           get_time[FFT_HARD][FFT_DIR_BACKWARD][TEST_START].tv_usec)));
  printf("[soft ][%d bytes][forward time = %ld us][backward time = %ld us]\n",
         FFT_N,
         (get_time[FFT_SOFT][FFT_DIR_FORWARD][TEST_STOP].tv_sec -
          get_time[FFT_SOFT][FFT_DIR_FORWARD][TEST_START].tv_sec) *
                 1000 * 1000 +
             (get_time[FFT_SOFT][FFT_DIR_FORWARD][TEST_STOP].tv_usec -
              get_time[FFT_SOFT][FFT_DIR_FORWARD][TEST_START].tv_usec),
         (get_time[FFT_SOFT][FFT_DIR_BACKWARD][TEST_STOP].tv_sec -
          get_time[FFT_SOFT][FFT_DIR_BACKWARD][TEST_START].tv_sec) *
                 1000 * 1000 +
             (get_time[FFT_SOFT][FFT_DIR_BACKWARD][TEST_STOP].tv_usec -
              get_time[FFT_SOFT][FFT_DIR_BACKWARD][TEST_START].tv_usec));
  while (1)
    ;
  return 0;
}
