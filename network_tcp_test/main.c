...
void dhcp_task(void* args) {
  ...
  switch (dhcp_state) {
    case DHCP_ADDRESS_ASSIGNED: {
      ...
      printf("get ipaddr:\n %d.%d.%d.%d \n", ipaddr.data[0], ipaddr.data[1],
             ipaddr.data[2], ipaddr.data[3]);
      printf("get mask:\n %d.%d.%d.%d \n", netmask.data[0], netmask.data[1],
             netmask.data[2], netmask.data[3]);
      printf("get gw:\n %d.%d.%d.%d \n", gateway.data[0], gateway.data[1],
             gateway.data[2], gateway.data[3]);
    } break;
    case DHCP_TIMEOUT: {
      printf("dhcp timeout \n");
    } break;
...
void init_network() {
  ...
  network_init();
  ip_address_t ipaddr = {.family = AF_INTERNETWORK,
                         .data = {192, 168, 190, 30}};
  ip_address_t netmask = {.family = AF_INTERNETWORK,
                          .data = {255, 255, 255, 0}};
  ip_address_t gateway = {.family = AF_INTERNETWORK,
                          .data = {192, 168, 190, 1}};
...
void tcp_client_task(void) {
  ...
  network_socket_send(sock, (const uint8_t*)buffer, strlen(buffer),
                      MESSAGE_NORMAL);
...
void tcp_server_task() {
  ...
  while (1) {
    ...
    receive_size = network_socket_receive(client_sock, (uint8_t*)buffer,
                                          sizeof(buffer), MESSAGE_NORMAL);
