...
void tftp_get(char* server_ip, unsigned short port, char* remote_file,
  ...
  sendto(sock, &snd_packet, k_tftp_packet_size, 0, (struct sockaddr*)&server,
         addr_len);
...
void tftp_put(char* server_ip, unsigned short port, char* local_file) {
  ...
  for (time_wait_ack = 0; time_wait_ack < PKT_RCV_TIMEOUT;
    ...
    if (rcv_size >= 4 && rcv_packet.cmd == htons((uint16_t)CMD_ACK) &&
        rcv_packet.block == htons(0)) {
      break;
    }
    usleep(20000);
  }
  ...
  do {
    ...
    for (rxmt = 0; rxmt < PKT_MAX_RXMT; rxmt++) {
      sendto(sock, &snd_packet, snd_size + 4, 0, (struct sockaddr*)&sender,
             addr_len);
      K_TFTP_DBG("Send %d\n", block);
