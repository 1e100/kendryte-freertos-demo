...
struct tensor_static_symgroup_permutate<numeric_list<int, nn...>> {
  ...
  template <typename T>
  constexpr static inline std::array<T, N> run(
...
struct tensor_static_symgroup_multiply_helper {
  template <int... iia>
  constexpr static inline numeric_list<int, get<iia, iib>::value...> helper(
      numeric_list<int, iia...>) {
...
static inline std::vector<Index> tensor_static_symgroup_index_permute(
    std::vector<Index> idx, internal::numeric_list<int, ii...>) {
