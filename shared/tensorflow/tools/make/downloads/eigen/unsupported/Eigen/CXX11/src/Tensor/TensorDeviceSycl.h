...
struct CheckAlignStatically {
  static const bool Val =
      (((Align & (Align - 1)) == 0) && (Align >= sizeof(void*)));
};
...
template <typename Scalar, size_t Align = EIGEN_MAX_ALIGN_BYTES,
...
struct SyclAllocator {
  ...
  Scalar* allocate(std::size_t elements) {
    return static_cast<Scalar*>(
        Conditional_Allocate<CheckAlignStatically<Align>::Val,
                             Align>::conditional_allocate(elements));
  }
  void deallocate(Scalar* p, std::size_t size) {
...
#define ConvertToActualTypeSycl(Scalar, buf_acc) \
...
template <typename Scalar, typename read_accessor, typename write_accessor>
class MemCopyFunctor {
 public:
  MemCopyFunctor(read_accessor src_acc, write_accessor dst_acc, size_t rng,
...
struct memsetkernelFunctor {
  ...
  memsetkernelFunctor(AccType acc, const ptrdiff_t buff_offset_,
                      const size_t rng, const size_t c)
      : m_acc(acc), buff_offset(buff_offset_), m_rng(rng), m_c(c) {}
  void operator()(cl::sycl::nd_item<1> itemID) {
    auto globalid = itemID.get_global_linear_id();
...
struct memsetCghFunctor {
  cl::sycl::buffer<uint8_t, 1, SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>&
      m_buf;
  const ptrdiff_t& buff_offset;
  ...
  memsetCghFunctor(
      cl::sycl::buffer<uint8_t, 1,
                       SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>& buff,
      const ptrdiff_t& buff_offset_, const size_t& rng_, const size_t& GRange_,
      const size_t& tileSize_, const int& c_)
      : m_buf(buff),
        buff_offset(buff_offset_),
        rng(rng_),
        GRange(GRange_),
        tileSize(tileSize_),
        c(c_) {}
  ...
  void operator()(cl::sycl::handler& cgh) const {
    auto buf_acc =
        m_buf.template get_access<cl::sycl::access::mode::write,
                                  cl::sycl::access::target::global_buffer>(cgh);
    typedef decltype(buf_acc) AccType;
    cgh.parallel_for(
        cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange),
                              cl::sycl::range<1>(tileSize)),
        memsetkernelFunctor<AccType>(buf_acc, buff_offset, rng, c));
  }
};

// get_devices returns all the available opencl devices. Either use
// device_selector or exclude devices that computecpp does not support (AMD
// OpenCL for CPU  and intel GPU)
...
class QueueInterface {
 public:
  /// creating device by using cl::sycl::selector or cl::sycl::device both are
  /// the same and can be captured through dev_Selector typename
  /// SyclStreamDevice is not owned. it is the caller's responsibility to
  /// destroy it.
  template <typename dev_Selector>
  explicit QueueInterface(const dev_Selector& s)
      :
...
#else
        m_queue(cl::sycl::queue(
            s,
            [&](cl::sycl::exception_list l) {
              for (const auto& e : l) {
                if (e) {
                  exception_caught_ = true;
                  std::cerr << "Error detected Inside Sycl Device."
                            << std::endl;
                }
              }
            }))
  ...
  EIGEN_STRONG_INLINE void* allocate(size_t num_bytes) const {
    ...
    auto buf = cl::sycl::buffer<uint8_t, 1,
                                SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>(
        cl::sycl::range<1>(num_bytes));
    auto ptr = buf.get_access<cl::sycl::access::mode::discard_write,
    ...
    buffer_map.insert(
        std::pair<
            ...
            cl::sycl::buffer<uint8_t, 1,
                             SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>>(
            static_cast<const uint8_t*>(ptr), buf));
  ...
  /// The memcpyHostToDevice is used to copy the device only pointer to a host
  /// pointer. Using the device pointer created as a key we find the sycl buffer
  /// and get the host accessor with write mode on it. Then we use the memcpy to
  /// copy the data to the host accessor. The first time that this buffer is
  /// accessed, the data will be copied to the device. In this case we can
  /// separate the kernel actual execution from data transfer which is required
  /// for benchmark Also, this is faster as it uses the map_allocator instead of
  /// memcpy
  template <typename Index>
  EIGEN_STRONG_INLINE void memcpyHostToDevice(Index* dst, const Index* src,
                                              size_t n) const {
    auto it = find_buffer(dst);
    auto offset =
        static_cast<const uint8_t*>(static_cast<const void*>(dst)) - it->first;
    offset /= sizeof(Index);
    size_t rng, GRange, tileSize;
    parallel_for_setup(n / sizeof(Index), tileSize, rng, GRange);
    auto src_buf =
        cl::sycl::buffer<uint8_t, 1, cl::sycl::map_allocator<uint8_t>>(
            static_cast<uint8_t*>(static_cast<void*>(const_cast<Index*>(src))),
            cl::sycl::range<1>(n));
    m_queue.submit([&](cl::sycl::handler& cgh) {
      ...
      cgh.parallel_for(cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange),
                                             cl::sycl::range<1>(tileSize)),
                       MemCopyFunctor<Index, read_accessor, write_accessor>(
                           src_acc, dst_acc, rng, offset, 0));
    });
    synchronize();
  }
  /// The memcpyDeviceToHost is used to copy the data from host to device. Here,
  /// in order to avoid double copying the data. We create a sycl buffer with
  /// map_allocator for the destination pointer with a discard_write accessor on
  /// it. The lifespan of the buffer is bound to the lifespan of the
  /// memcpyDeviceToHost function. We create a kernel to copy the data, from the
  /// device- only source buffer to the destination buffer with map_allocator on
  /// the gpu in parallel. At the end of the function call the destination
  /// buffer would be destroyed and the data would be available on the dst
  /// pointer using fast copy technique (map_allocator). In this case we can
  /// make sure that we copy the data back to the cpu only once per function
  /// call.
  template <typename Index>
  EIGEN_STRONG_INLINE void memcpyDeviceToHost(void* dst, const Index* src,
                                              size_t n) const {
    auto it = find_buffer(src);
    auto offset =
        static_cast<const uint8_t*>(static_cast<const void*>(src)) - it->first;
    offset /= sizeof(Index);
    size_t rng, GRange, tileSize;
    parallel_for_setup(n / sizeof(Index), tileSize, rng, GRange);
    auto dest_buf =
        cl::sycl::buffer<uint8_t, 1, cl::sycl::map_allocator<uint8_t>>(
            static_cast<uint8_t*>(dst), cl::sycl::range<1>(n));
    m_queue.submit([&](cl::sycl::handler& cgh) {
      auto src_acc =
          it->second
              .template get_access<cl::sycl::access::mode::read,
                  ...
                  cgh);
      auto dst_acc =
          dest_buf.template get_access<cl::sycl::access::mode::discard_write,
                                       cl::sycl::access::target::global_buffer>(
              cgh);
      typedef decltype(src_acc) read_accessor;
      typedef decltype(dst_acc) write_accessor;
  ...
  EIGEN_STRONG_INLINE void memcpy(void* dst, const Index* src, size_t n) const {
    ...
    auto offset = (static_cast<const uint8_t*>(static_cast<const void*>(src))) -
                  it1->first;
  ...
  EIGEN_STRONG_INLINE void memset(void* data, int c, size_t n) const {
    ...
    m_queue.submit(
        memsetCghFunctor(it1->second, buff_offset, rng, GRange, tileSize, c));
    synchronize();
  }
  ...
  EIGEN_STRONG_INLINE cl::sycl::accessor<
      uint8_t, 1, AcMd, cl::sycl::access::target::global_buffer>
  ...
  EIGEN_STRONG_INLINE cl::sycl::buffer<
      uint8_t, 1, SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>&
  ...
  EIGEN_STRONG_INLINE void asynchronousExec() const {
    /// FIXEDME:: currently there is a race condition regarding the asynch
    /// scheduler.
    // sycl_queue().throw_asynchronous();// FIXME::does not pass. Temporarily
    // disabled
    m_queue.wait_and_throw();  // pass
  }
  ...
  EIGEN_STRONG_INLINE void parallel_for_setup(Index n, Index& tileSize,
    ...
    tileSize = static_cast<Index>(
        m_queue.get_device()
            .template get_info<cl::sycl::info::device::max_work_group_size>());
    auto s = m_queue.get_device()
                 .template get_info<cl::sycl::info::device::vendor>();
    std::transform(s.begin(), s.end(), s.begin(), ::tolower);
    if (m_queue.get_device()
            .is_cpu()) {  // intel doesn't allow to use max workgroup size
      tileSize =
          std::min(static_cast<Index>(256), static_cast<Index>(tileSize));
    }
    rng = n;
  ...
  EIGEN_STRONG_INLINE void parallel_for_setup(Index dim0, Index dim1,
    ...
    if (m_queue.get_device()
            .is_cpu()) {  // intel doesn't allow to use max workgroup size
      max_workgroup_Size = std::min(static_cast<Index>(256),
 ...
 private:
  ...
  EIGEN_STRONG_INLINE std::map<
      const uint8_t*,
      cl::sycl::buffer<uint8_t, 1,
                       SyclAllocator<uint8_t, EIGEN_MAX_ALIGN_BYTES>>>::iterator
  find_buffer(const void* ptr) const {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (it1 != buffer_map.end()) {
      ...
      for (std::map<
           ...
           it != buffer_map.end(); ++it) {
        auto size = it->second.get_size();
        if ((it->first < (static_cast<const uint8_t*>(ptr))) &&
            ((static_cast<const uint8_t*>(ptr)) < (it->first + size)))
          return it;
      }
    }
    std::cerr << "No sycl buffer found. Make sure that you have allocated "
                 "memory for your buffer by calling malloc-ed function."
              << std::endl;
    abort();
  }
};
...
struct SyclDevice {
  ...
  /// it.
  explicit SyclDevice(QueueInterface* queue_stream)
  ...
  EIGEN_STRONG_INLINE void parallel_for_setup(Index dim0, Index dim1,
                                              ...
                                              Index& rng1, Index& GRange0,
                                              Index& GRange1) const {
    m_queue_stream->parallel_for_setup(dim0, dim1, tileSize0, tileSize1, rng0,
  ...
  EIGEN_STRONG_INLINE void parallel_for_setup(
      ...
      Index& tileSize2, Index& rng0, Index& rng1, Index& rng2, Index& GRange0,
      Index& GRange1, Index& GRange2) const {
    m_queue_stream->parallel_for_setup(dim0, dim1, dim2, tileSize0, tileSize1,
  ...
  EIGEN_STRONG_INLINE cl::sycl::queue& sycl_queue() const {
    return m_queue_stream->sycl_queue();
  }
  ...
  EIGEN_STRONG_INLINE int majorDeviceVersion() const {
    return m_queue_stream->majorDeviceVersion();
  }
  ...
  // This function checks if the runtime recorded an error for the
  // underlying stream device.
  EIGEN_STRONG_INLINE bool ok() const { return m_queue_stream->ok(); }
};
// This is used as a distingushable device inside the kernel as the sycl device
// class is not Standard layout. This is internal and must not be used by user.
// This dummy device allow us to specialise the tensor evaluator inside the
// kernel. So we can have two types of eval for host and device. This is
// required for TensorArgMax operation
