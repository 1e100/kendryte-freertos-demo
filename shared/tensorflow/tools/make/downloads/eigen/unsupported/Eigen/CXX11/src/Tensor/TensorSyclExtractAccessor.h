...
struct AccessorConstructor {
  ...
  static inline auto getTuple(cl::sycl::handler& cgh, const Arg& eval)
      RETURN_CPP11(ExtractAccessor<Arg>::getTuple(cgh, eval))
          ...
          static inline auto getTuple(cl::sycl::handler& cgh, const Arg1& eval1,
                                      const Arg2& eval2)
              RETURN_CPP11(utility::tuple::append(
                  ...
                  static inline auto getTuple(cl::sycl::handler& cgh,
                      ...
                      RETURN_CPP11(utility::tuple::append(
                          ...
                          static inline auto getAccessor(cl::sycl::handler& cgh,
                                                         const Arg& eval)
                              RETURN_CPP11(utility::tuple::make_tuple(
                                  eval.device().template get_sycl_accessor<AcM>(
                                      cgh, eval.data())))
};
...
#define SYCLUNARYCATEGORYEXTACC(CVQual)                                      \
  ...
  struct ExtractAccessor<                                                    \
    ...
    static inline auto getTuple(                                             \
        cl::sycl::handler& cgh,                                              \
        const TensorEvaluator<CVQual UnaryCategory<OP, RHSExpr>, Dev>& eval) \
...
#define SYCLBINARYCATEGORYEXTACC(CVQual)                                      \
  ...
  struct ExtractAccessor<                                                     \
    ...
    static inline auto getTuple(                                              \
        cl::sycl::handler& cgh,                                               \
        const TensorEvaluator<CVQual BinaryCategory<OP, LHSExpr, RHSExpr>,    \
                              Dev>& eval)                                     \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.left_impl(),     \
                                                   eval.right_impl()))        \
  };
  ...
  struct ExtractAccessor<TensorEvaluator<                                   \
    ...
    static inline auto getTuple(                                            \
        ...
        const TensorEvaluator<                                              \
            CVQual TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>, Dev>& \
            eval)                                                           \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.arg1Impl(),    \
                                                   eval.arg2Impl(),         \
                                                   eval.arg3Impl()))        \
  };
...
#define SYCLSELECTOPEXTACC(CVQual)                                         \
  ...
  struct ExtractAccessor<TensorEvaluator<                                  \
    ...
    static inline auto getTuple(                                           \
        cl::sycl::handler& cgh,                                            \
        const TensorEvaluator<                                             \
            CVQual TensorSelectOp<IfExpr, ThenExpr, ElseExpr>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.cond_impl(),  \
                                                   eval.then_impl(),       \
                                                   eval.else_impl()))      \
  };
  ...
  struct ExtractAccessor<                                                      \
    ...
    static inline auto getTuple(                                               \
        ...
        const TensorEvaluator<CVQual TensorMap<PlainObjectType, Options_>,     \
                              Dev>& eval)                                      \
        RETURN_CPP11(AccessorConstructor::template getAccessor<ACCType>(cgh,   \
                                                                        eval)) \
  };
...
#define SYCLFORCEDEVALEXTACC(CVQual)                                       \
  ...
  struct ExtractAccessor<                                                  \
      TensorEvaluator<CVQual TensorForcedEvalOp<Expr>, Dev> > {            \
    static inline auto getTuple(                                           \
        cl::sycl::handler& cgh,                                            \
        const TensorEvaluator<CVQual TensorForcedEvalOp<Expr>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::template getAccessor<            \
...
#define SYCLCUSTOMUNARYOPEXTACC(CVQual)                                       \
  ...
  struct ExtractAccessor<TensorEvaluator<                                     \
      CVQual TensorCustomUnaryOp<CustomUnaryFunc, XprType>, Dev> > {          \
    static inline auto getTuple(                                              \
        cl::sycl::handler& cgh,                                               \
        const TensorEvaluator<                                                \
            CVQual TensorCustomUnaryOp<CustomUnaryFunc, XprType>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::template getAccessor<               \
                     cl::sycl::access::mode::read>(cgh, eval))                \
  };
...
#define SYCLCUSTOMBINARYOPEXTACC(CVQual)                                     \
  template <typename CustomBinaryFunc, typename LhsXprType,                  \
            typename RhsXprType, typename Dev>                               \
  struct ExtractAccessor<TensorEvaluator<                                    \
      CVQual TensorCustomBinaryOp<CustomBinaryFunc, LhsXprType, RhsXprType>, \
      Dev> > {                                                               \
    static inline auto getTuple(                                             \
        cl::sycl::handler& cgh,                                              \
        const TensorEvaluator<CVQual TensorCustomBinaryOp<                   \
                                  CustomBinaryFunc, LhsXprType, RhsXprType>, \
                              Dev>& eval)                                    \
        RETURN_CPP11(AccessorConstructor::template getAccessor<              \
                     cl::sycl::access::mode::read>(cgh, eval))               \
  };
...
#define SYCLEVALTOEXTACC(CVQual)                                               \
  ...
  struct ExtractAccessor<TensorEvaluator<CVQual TensorEvalToOp<Expr>, Dev> > { \
    static inline auto getTuple(                                               \
        cl::sycl::handler& cgh,                                                \
        const TensorEvaluator<CVQual TensorEvalToOp<Expr>, Dev>& eval)         \
        RETURN_CPP11(utility::tuple::append(                                   \
...
#define SYCLREDUCTIONEXTACC(CVQual, ExprNode)                             \
  ...
  struct ExtractAccessor<                                                 \
      TensorEvaluator<CVQual ExprNode<OP, Dim, Expr>, Dev> > {            \
    static inline auto getTuple(                                          \
        cl::sycl::handler& cgh,                                           \
        const TensorEvaluator<CVQual ExprNode<OP, Dim, Expr>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::template getAccessor<           \
                     cl::sycl::access::mode::read>(cgh, eval))            \
  };
// TensorReductionOp
...
#define SYCLCONTRACTIONCONVOLUTIONEXTACC(CVQual, ExprNode)                \
  template <typename Indices, typename LhsXprType, typename RhsXprType,   \
            typename Dev>                                                 \
  struct ExtractAccessor<TensorEvaluator<                                 \
      CVQual ExprNode<Indices, LhsXprType, RhsXprType>, Dev> > {          \
    static inline auto getTuple(                                          \
        cl::sycl::handler& cgh,                                           \
        const TensorEvaluator<                                            \
            CVQual ExprNode<Indices, LhsXprType, RhsXprType>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::template getAccessor<           \
                     cl::sycl::access::mode::read>(cgh, eval))            \
  };
// TensorContractionOp
...
#define SYCLSLICEOPEXTACC(CVQual)                                             \
  ...
  struct ExtractAccessor<TensorEvaluator<                                     \
    ...
    static inline auto getTuple(                                              \
        cl::sycl::handler& cgh,                                               \
        const TensorEvaluator<                                                \
            CVQual TensorSlicingOp<StartIndices, Sizes, XprType>, Dev>& eval) \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.impl()))         \
  };
...
#define SYCLTENSORCHIPPINGOPEXTACC(CVQual)                                   \
  ...
  struct ExtractAccessor<                                                    \
      TensorEvaluator<CVQual TensorChippingOp<DimId, XprType>, Dev> > {      \
    static inline auto getTuple(                                             \
        cl::sycl::handler& cgh,                                              \
        const TensorEvaluator<CVQual TensorChippingOp<DimId, XprType>, Dev>& \
            eval)                                                            \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.impl()))        \
  };
...
#define SYCLTENSORIMAGEPATCHOPEXTACC(CVQual)                                   \
  ...
  struct ExtractAccessor<                                                      \
      TensorEvaluator<CVQual TensorImagePatchOp<Rows, Cols, XprType>, Dev> > { \
    static inline auto getTuple(                                               \
        cl::sycl::handler& cgh,                                                \
        const TensorEvaluator<CVQual TensorImagePatchOp<Rows, Cols, XprType>,  \
                              Dev>& eval)                                      \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.impl()))          \
  };
...
#define SYCLTENSORVOLUMEPATCHOPEXTACC(CVQual)                              \
  template <DenseIndex Planes, DenseIndex Rows, DenseIndex Cols,           \
            typename XprType, typename Dev>                                \
  struct ExtractAccessor<TensorEvaluator<                                  \
      CVQual TensorVolumePatchOp<Planes, Rows, Cols, XprType>, Dev> > {    \
    static inline auto getTuple(                                           \
        cl::sycl::handler& cgh,                                            \
        const TensorEvaluator<                                             \
            CVQual TensorVolumePatchOp<Planes, Rows, Cols, XprType>, Dev>& \
...
#define SYCLTENSORLAYOUTSWAPINDEXTUPLEOPEXTACC(CVQual, ExprNode)            \
  ...
  struct ExtractAccessor<TensorEvaluator<CVQual ExprNode<XprType>, Dev> > { \
    static inline auto getTuple(                                            \
        cl::sycl::handler& cgh,                                             \
        const TensorEvaluator<CVQual ExprNode<XprType>, Dev>& eval)         \
        RETURN_CPP11(AccessorConstructor::getTuple(cgh, eval.impl()))       \
  };
