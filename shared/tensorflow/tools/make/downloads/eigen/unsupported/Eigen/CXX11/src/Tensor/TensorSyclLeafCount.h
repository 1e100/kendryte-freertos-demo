...
#define SYCLTENSORMAPLEAFCOUNT(CVQual)                              \
  ...
  struct LeafCount<                                                 \
      CVQual TensorMap<PlainObjectType, Options_, MakePointer_> > { \
...
#define SYCLCATEGORYLEAFCOUNT(CVQual)                                   \
  template <template <class, class...> class CategoryExpr, typename OP, \
            typename... RHSExpr>                                        \
  struct LeafCount<CVQual CategoryExpr<OP, RHSExpr...> >                \
...
#define SYCLSELECTOPLEAFCOUNT(CVQual)                                  \
  ...
  struct LeafCount<CVQual TensorSelectOp<IfExpr, ThenExpr, ElseExpr> > \
...
#define SYCLLEAFCOUNTASSIGNOP(CVQual)                        \
  ...
  struct LeafCount<CVQual TensorAssignOp<LHSExpr, RHSExpr> > \
...
#define SYCLCUSTOMBINARYOPLEAFCOUNT(CVQual)                                  \
  template <typename CustomBinaryFunc, typename LhsXprType,                  \
            typename RhsXprType>                                             \
  struct LeafCount<CVQual TensorCustomBinaryOp<CustomBinaryFunc, LhsXprType, \
                                               RhsXprType> > {               \
    static const size_t Count = 1;                                           \
  };
...
#define SLICEOPLEAFCOUNT(CVQual)                                          \
  ...
  struct LeafCount<CVQual TensorSlicingOp<StartIndices, Sizes, XprType> > \
      : CategoryCount<XprType> {};
...
#define CHIPPINGOPLEAFCOUNT(CVQual)                          \
  ...
  struct LeafCount<CVQual TensorChippingOp<DimId, XprType> > \
      : CategoryCount<XprType> {};
...
#define SLICESTRIDEOPLEAFCOUNT(CVQual)                                       \
  template <typename StartIndices, typename StopIndices, typename Strides,   \
            typename XprType>                                                \
  struct LeafCount<CVQual TensorStridingSlicingOp<StartIndices, StopIndices, \
                                                  Strides, XprType> >        \
      : CategoryCount<XprType> {};
...
#define TENSORIMAGEPATCHOPLEAFCOUNT(CVQual)                         \
  ...
  struct LeafCount<CVQual TensorImagePatchOp<Rows, Cols, XprType> > \
      : CategoryCount<XprType> {};
...
#define TENSORVOLUMEPATCHOPLEAFCOUNT(CVQual)                                 \
  template <DenseIndex Planes, DenseIndex Rows, DenseIndex Cols,             \
            typename XprType>                                                \
  struct LeafCount<CVQual TensorVolumePatchOp<Planes, Rows, Cols, XprType> > \
      : CategoryCount<XprType> {};
