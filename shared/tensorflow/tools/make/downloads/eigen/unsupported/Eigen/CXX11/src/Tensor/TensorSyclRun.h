...
struct ExecExprFunctorKernel {
  ...
  Index range;
  ExecExprFunctorKernel(Index range_, FunctorExpr functors_,
  ...
  void operator()(cl::sycl::nd_item<1> itemID) {
    ...
    typename DevExpr::Index gId =
        static_cast<typename DevExpr::Index>(itemID.get_global_linear_id());
...
/// std::array does not have TotalSize. So I have to get the size through
...
#define DIMSIZEMACRO(CVQual)                                                \
  ...
  struct DimensionSize<Index, CVQual std::array<Index, NumDims>> {          \
...
void run(Expr& expr, Dev& dev) {
  ...
  if (needs_assign) {
    typedef Eigen::TensorSycl::internal::FunctorExtractor<
        Eigen::TensorEvaluator<Expr, Dev>>
        FunctorExpr;
    FunctorExpr functors = internal::extractFunctors(evaluator);
    dev.sycl_queue().submit([&](cl::sycl::handler& cgh) {
      // create a tuple of accessors from Evaluator
      typedef decltype(
          internal::createTupleOfAccessors<Eigen::TensorEvaluator<Expr, Dev>>(
              cgh, evaluator)) TupleType;
      TupleType tuple_of_accessors =
          internal::createTupleOfAccessors<Eigen::TensorEvaluator<Expr, Dev>>(
              cgh, evaluator);
      typename Expr::Index range, GRange, tileSize;
      typename Expr::Index total_size = static_cast<typename Expr::Index>(
          DimensionSize<typename Expr::Index,
                        typename Eigen::TensorEvaluator<Expr, Dev>::
                            Dimensions>::getDimSize(evaluator.dimensions()));
      ...
      cgh.parallel_for(cl::sycl::nd_range<1>(cl::sycl::range<1>(GRange),
                       ...
                       ExecExprFunctorKernel<Expr, FunctorExpr, TupleType>(
                           range, functors, tuple_of_accessors));
    });
