...
int main(void) {
  ...
  *(uint32_t*)(&slave_cfg[8]) = (uint32_t)test_data_tmp;
  ...
  printf("System start\n");
#ifdef MASTER_MODE
  for (uint32_t i = 0; i < 8; i++) test_data[i] = i;
  spi_master_transfer(test_data, 0, 8, WRITE_CONFIG);
  for (uint32_t i = 0; i < 8; i++) test_data[i] = 0;
  spi_master_transfer(test_data, 0, 8, READ_CONFIG);
  for (uint32_t i = 0; i < 8; i++) {
    if (test_data[i] != (uint8_t)i) printf("%d: 0x%02x,", i, test_data[i]);
  }
  printf("\n");
  ...
  for (uint32_t i = 0; i < 1024; i++) test_data[i] = i;
  spi_master_transfer(test_data, addr, 1024, WRITE_DATA_BLOCK);
  for (uint32_t i = 0; i < 1024; i++) test_data[i] = 0;
