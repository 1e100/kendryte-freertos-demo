...
int spi_master_init(void) {
  ...
  gpio_set_on_changed(gio, SPI_MASTER_INT_IO,
                      (gpio_on_changed_t)spi_slave_ready_irq, NULL);
...
static int spi_master_send_cmd(spi_slave_command_t* cmd) {
  ...
  for (uint32_t i = 0; i < WAIT_TIMEOUT; i++) {
    if (gpio_get_pin_value(gio, SPI_MASTER_INT_IO) == 1) break;
  }
  if (gpio_get_pin_value(gio, SPI_MASTER_INT_IO) == 0) return -1;
  ...
  data[6] = cmd->len >> 8;
  data[7] = 0;
  for (uint32_t i = 0; i < 7; i++) data[7] += data[i];
  spi_slave_ready = 0;
  spi_send_data(data, 8);
  for (uint32_t i = 0; i < WAIT_TIMEOUT; i++) {
    if (spi_slave_ready != 0) break;
  }
  if (spi_slave_ready)
    return 0;
  else
...
int spi_master_transfer(uint8_t* data, uint32_t addr, uint32_t len,
  ...
  cmd.cmd = mode;
  cmd.addr = addr;
  if (spi_master_send_cmd(&cmd) != 0) return -2;
  if (mode & 0x01)
